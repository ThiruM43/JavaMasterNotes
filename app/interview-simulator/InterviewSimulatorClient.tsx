  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`${base}/data/section-24-interview-prep.json`);
        if (res.ok) {
    ...
    return (
      <AppShell>
        <div className="empty-state" style={{ marginTop: 80 }}>
          ...
          <h3>Could not load Interview Simulator data</h3>
        </div>
      </AppShell>
    );
  }
    ...
    <AppShell>
      <div style={{ padding: '24px 16px', maxWidth: 1040, margin: '0 auto' }}>
        <div className="breadcrumb" style={{ marginBottom: 16 }}>
          ...
          <span style={{ color: 'var(--text-primary)' }}>🎯 Interview Simulator</span>
        </div>

        <div style={{
          background: 'linear-gradient(135deg, var(--surface) 0%, rgba(244, 63, 94, 0.12) 100%)',
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
            <div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(244, 63, 94, 0.15)', color: '#f43f5e', padding: '4px 12px', borderRadius: 20, fontSize: '0.75rem', fontWeight: 700, marginBottom: 12 }}>
                ⚡ SENIOR ARCHITECT INTERVIEW SIMULATOR
              </div>
              <h1 style={{ fontSize: '1.75rem', fontWeight: 800, margin: '0 0 8px 0', color: 'var(--text-primary)' }}>
                Master Your Narrative Flow
              </h1>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.92rem', maxWidth: 680, margin: 0, lineHeight: 1.5 }}>
                Instead of technical definitions, this simulator trains you on how to deliver your answers in a high-stakes interview. Compare this with the standard Guided Study Path.
              </p>
            </div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8 }}>
                <span style={{ fontSize: '1.6rem', fontWeight: 800, color: '#f43f5e' }}>{totalAllPct}%</span>
                <span style={{ fontSize: '0.82rem', color: 'var(--text-secondary)' }}>({totalAllRead} / {totalAllTerms} mastered)</span>
              </div>
              <div style={{ height: 6, background: 'var(--border-subtle)', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ height: '100%', background: '#f43f5e', width: `${totalAllPct}%`, transition: 'width 0.4s ease' }} />
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ display: 'flex', background: 'var(--bg-secondary)', borderRadius: 10, padding: 4, border: '1px solid var(--border-subtle)' }}>
              <button
                ...
                style={{
                  background: viewMode === 'focus' ? '#f43f5e' : 'transparent',
              ...
              <button
                ...
                style={{
                  background: viewMode === 'list' ? '#f43f5e' : 'transparent',
                ...
                return (
                  <button
                    ...
                    style={{
                      padding: '8px 14px',
                      borderRadius: 10,
                      border: isCurrent ? '2px solid #f43f5e' : '1px solid var(--border)',
                      background: isCurrent ? 'rgba(244, 63, 94, 0.12)' : isRead ? 'rgba(16, 185, 129, 0.08)' : 'var(--surface)',
                      color: isCurrent ? '#f43f5e' : isRead ? '#10b981' : 'var(--text-secondary)',
                      fontSize: '0.8rem',
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ background: 'var(--surface-raised)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 20px', marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <button onClick={() => handleMarkAndNext(currentTerm.id)} style={{ padding: '9px 20px', borderRadius: 8, border: 'none', background: '#f43f5e', color: '#ffffff', fontSize: '0.88rem', fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 12px rgba(244, 63, 94, 0.3)', display: 'flex', alignItems: 'center', gap: 6 }}>
