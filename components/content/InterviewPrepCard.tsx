...
import type { InterviewTerm } from '@/lib/types';
...
export function InterviewPrepCard({ term, sectionId, subsectionId, sectionTitle }: InterviewPrepCardProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 32 }}>
        ...
        {term.coreAnswer && (
          <section>
            <h3 style={{ fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '1px', color: '#f43f5e', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700 }}>
              <span>🗣️</span> The Pitch (Core Answer)
            </h3>
            <div style={{ fontSize: '1.05rem', color: 'var(--text-primary)' }}>
              <HighlightableField text={term.coreAnswer} field="coreAnswer" termId={term.id} sectionId={sectionId} termName={term.topic} sectionTitle={sectionTitle} />
            </div>
          </section>
        )}

        {term.architectureDetails && (
          <section style={{ background: 'var(--bg-secondary)', padding: '20px', borderRadius: 12, border: '1px solid var(--border-subtle)' }}>
            <h3 style={{ fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '1px', color: '#10b981', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700 }}>
              <span>🏗️</span> Architecture & Implementation
            </h3>
            <div style={{ fontSize: '0.95rem', color: 'var(--text-secondary)' }}>
              <HighlightableField text={term.architectureDetails} field="architectureDetails" termId={term.id} sectionId={sectionId} termName={term.topic} sectionTitle={sectionTitle} />
            </div>
          </section>
        )}

        {term.tradeOffs && (
          <section style={{ borderLeft: '3px solid #3b82f6', paddingLeft: '16px' }}>
            <h3 style={{ fontSize: '0.85rem', color: '#3b82f6', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 6, fontWeight: 700 }}>
              <span>⚖️</span> Trade-offs & Decisions
            </h3>
            <div style={{ fontSize: '0.95rem', color: 'var(--text-secondary)' }}>
              <HighlightableField text={term.tradeOffs} field="tradeOffs" termId={term.id} sectionId={sectionId} termName={term.topic} sectionTitle={sectionTitle} />
            </div>
          </section>
        )}

        {term.realWorldScenarios && (
          <section style={{ background: 'rgba(245, 158, 11, 0.04)', border: '1px solid rgba(245, 158, 11, 0.2)', padding: '20px', borderRadius: 12 }}>
            <h3 style={{ fontSize: '0.9rem', color: '#d97706', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700 }}>
              <span>⚠️</span> Real-World Scenarios & Troubleshooting
            </h3>
            <div style={{ fontSize: '0.95rem', color: 'var(--text-primary)' }}>
              <HighlightableField text={term.realWorldScenarios} field="realWorldScenarios" termId={term.id} sectionId={sectionId} termName={term.topic} sectionTitle={sectionTitle} />
            </div>
          </section>
        )}
        ...
        <div style={{ padding: '0 24px 24px 24px' }}>
          <div style={{ background: 'var(--bg-secondary)', padding: '16px', borderRadius: 12, border: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              ...
              <button onClick={handleAddNote} style={{ padding: '8px 16px', borderRadius: 6, border: 'none', background: '#f43f5e', color: '#fff', fontWeight: 600, cursor: 'pointer' }}>Save Note</button>
            </div>
          </div>
        </div>
      )}
